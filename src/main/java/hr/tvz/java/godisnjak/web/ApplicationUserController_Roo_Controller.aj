...
import hr.tvz.java.godisnjak.web.ApplicationUserController;
...
privileged aspect ApplicationUserController_Roo_Controller {
    ...
    public String ApplicationUserController.show(@PathVariable("id") Long id, Model uiModel) {
        uiModel.addAttribute("applicationuser", ApplicationUser.findApplicationUser(id));
        uiModel.addAttribute("itemId", id);
        return "applicationusers/show";
    }
    
    @RequestMapping(produces = "text/html")
    public String ApplicationUserController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "sortFieldName", required = false) String sortFieldName, @RequestParam(value = "sortOrder", required = false) String sortOrder, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("applicationusers", ApplicationUser.findApplicationUserEntries(firstResult, sizeNo, sortFieldName, sortOrder));
            float nrOfPages = (float) ApplicationUser.countApplicationUsers() / sizeNo;
            uiModel.addAttribute("maxPages", (int) ((nrOfPages > (int) nrOfPages || nrOfPages == 0.0) ? nrOfPages + 1 : nrOfPages));
        } else {
            uiModel.addAttribute("applicationusers", ApplicationUser.findAllApplicationUsers(sortFieldName, sortOrder));
        }
    ...
    public String ApplicationUserController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        ApplicationUser applicationUser = ApplicationUser.findApplicationUser(id);
    ...
    void ApplicationUserController.populateEditForm(Model uiModel, ApplicationUser applicationUser) {
        ...
        uiModel.addAttribute("applicationusertypes", ApplicationUserType.findAllApplicationUserTypes());
    }
